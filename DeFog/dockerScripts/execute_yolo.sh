...
function which_program_edge_split_pipeline {
	...
	then
		...
		executeApplication="./darknet detect cfg/yolov3-tiny.cfg yolov3-tiny.weights /mnt/assets/$imageName.jpg"
...
function set_application {
	application=$1
	echo $application we are from exectue.sh line 304
}

# set the input pipeline
function set_pipeline {
       	pipeline=$1
       	echo $pipeline we are from exectue.sh line 309
}

function set_imagename {
       imageName=$1
       echo $imageName we are from execute.sh line 314
}
...
while getopts :ljpy:bcez: opt; do
    case "$opt" in
        y)
            application=0
            imageName=$OPTARG
	    echo $imageName from execute.sh file
	    echo $OPTARG    from execute.sh file
            ;;
		l)
            application=3
            ;;
